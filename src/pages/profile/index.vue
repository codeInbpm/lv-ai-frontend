...
function chooseAvatar() {
  uni.chooseImage({
    ...
    success(res) {
      ...
      // TODO: 上传到OSS后更新
      userStore.updateProfile({ avatar: tempFilePath })
    }
  })
}
...
function goCollections() {
  uni.navigateTo({ url: '/pages/profile/collections' })
}
...
<template>
  <view class="profile-page">
    <!-- 顶部用户信息 -->
    <view class="user-header" :style="{ paddingTop: statusBarHeight + 20 + 'px' }">
      <view v-if="!userStore.isLogin" class="login-prompt" @click="uni.navigateTo({ url: '/pages/login/index' })">
        <view class="avatar-placeholder-lg">
          <text>👤</text>
        </view>
        <view>
        ...
        <view class="user-info">
          <view class="avatar-wrap" @click="chooseAvatar">
            <image
              ...
              mode="aspectFill"
            />
            <view v-else class="avatar-placeholder-lg">
              <text>👤</text>
            </view>
    ...
    <scroll-view class="scroll" scroll-y>
      ...
      <view class="menu-section card">
        ...
        <view class="menu-item" @click="uni.navigateTo({ url: '/pages/profile/footprints' })">
          <text class="menu-icon">🗺️</text>
